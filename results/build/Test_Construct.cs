...
// Command-line arguments: build src/Test_Construct.dfy src/Common.dfy src/QuotientGraph.dfy -o results/build/Test_Construct
// the_program
...
module Common {
  function Id(a: Atom): Vertex
    decreases a
  {
    Vertex(a.atomLabel, a.sym)
  }

  function AllNodeIds(atoms: seq<Atom>): set<Vertex>
    decreases atoms
  {
    set i: int {:trigger atoms[i]} | 0 <= i < |atoms| :: Id(atoms[i])
  }

  function Add(u: Translation, v: Translation): Translation
    decreases u, v
  {
    Translation(u.x + v.x, u.y + v.y, u.z + v.z)
  }

  function Subtract(u: Translation, v: Translation): Translation
    decreases u, v
  {
    Translation(u.x - v.x, u.y - v.y, u.z - v.z)
  }

  function Negate(u: Translation): Translation
    decreases u
  {
  ...
  lemma IdentityVector()
    ensures forall v: Translation {:trigger Add(Translation(0, 0, 0), v)} {:trigger Add(v, Translation(0, 0, 0))} :: Add(v, Translation(0, 0, 0)) == v && Add(Translation(0, 0, 0), v) == v
  {
  }
...
module QuotientGraph {
  method ConstructLQG(atoms: seq<Atom>, bonds: seq<Bond>)
    ...
    requires |atoms| > 0
    requires forall b: Bond {:trigger b.b} {:trigger b.a} {:trigger b in bonds} :: (b in bonds ==> b.a in atoms) && (b in bonds ==> b.b in atoms)
    ensures forall e1: Edge, e2: Edge {:trigger e2.shift, e1.shift} {:trigger e2.shift, e1.v} {:trigger e2.shift, e1.u} {:trigger e2.shift, e1 in E} {:trigger e1.shift, e2.u} {:trigger e1.shift, e2.v} {:trigger e1.shift, e2 in E} {:trigger e2.u, e1.v} {:trigger e2.u, e1.u} {:trigger e2.u, e1 in E} {:trigger e1.v, e2.v} {:trigger e1.v, e2 in E} {:trigger e2.v, e1.u} {:trigger e2.v, e1 in E} {:trigger e1.u, e2 in E} {:trigger e2 in E, e1 in E} | e1 in E && e2 in E && e1 != e2 :: !(e1.u == e2.v && e1.v == e2.u && e1.shift == Negate(e2.shift))
    ensures forall i: int {:trigger atoms[i]} | 0 <= i < |atoms| :: Id(atoms[i]) in V
    ensures V == AllNodeIds(atoms)
    decreases atoms, bonds
  {
    ...
    var atomIdx := 0;
    while atomIdx < |atoms|
      invariant forall e1: Edge, e2: Edge {:trigger e2.shift, e1.shift} {:trigger e2.shift, e1.v} {:trigger e2.shift, e1.u} {:trigger e2.shift, e1 in edges} {:trigger e1.shift, e2.u} {:trigger e1.shift, e2.v} {:trigger e1.shift, e2 in edges} {:trigger e2.u, e1.v} {:trigger e2.u, e1.u} {:trigger e2.u, e1 in edges} {:trigger e1.v, e2.v} {:trigger e1.v, e2 in edges} {:trigger e2.v, e1.u} {:trigger e2.v, e1 in edges} {:trigger e1.u, e2 in edges} {:trigger e2 in edges, e1 in edges} | e1 in edges && e2 in edges && e1 != e2 :: !(e1.u == e2.v && e1.v == e2.u && e1.shift == Negate(e2.shift))
  ...
  function BaseAtom(v: Vertex): Atom
    decreases v
  {
    Atom(v.atomLabel, v.sym, Translation(0, 0, 0))
  }

  function ShiftedAtom(e: Edge): Atom
    decreases e
  {
    Atom(e.v.atomLabel, e.v.sym, e.shift)
  }

  function BaseAtoms(V: set<Vertex>): set<Atom>
    decreases V
  {
    set v: Vertex {:trigger BaseAtom(v)} {:trigger v in V} | v in V :: BaseAtom(v)
  }

  function TranslatedAtoms(E: set<Edge>): set<Atom>
    decreases E
  {
    set e: Edge {:trigger ShiftedAtom(e)} {:trigger e.shift} {:trigger e in E} | e in E && e.shift != Translation(0, 0, 0) :: ShiftedAtom(e)
  }

  method ReconstructMolecule(V: set<Vertex>, E: set<Edge>)
      returns (atoms: set<Atom>, bonds: set<Bond>)
    requires forall e: Edge {:trigger e.v} {:trigger e.u} {:trigger e in E} | e in E :: e.u in V && e.v in V
    ...
    decreases V, E
  {
    ...
    bonds := set e: Edge {:trigger ShiftedAtom(e)} {:trigger e.u} {:trigger e in E} | e in E :: Bond(BaseAtom(e.u), ShiftedAtom(e));
...
namespace Common {
  ...
  public partial class __default {
    public static Common._IVertex Id(Common._IAtom a) {
      return Common.Vertex.create((a).dtor_atomLabel, (a).dtor_sym);
    }
    public static Dafny.ISet<Common._IVertex> AllNodeIds(Dafny.ISequence<Common._IAtom> atoms) {
      return Dafny.Helpers.Id<Func<Dafny.ISequence<Common._IAtom>, Dafny.ISet<Common._IVertex>>>((_0_atoms) => ((System.Func<Dafny.ISet<Common._IVertex>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Common._IVertex>();
        foreach (BigInteger _compr_0 in Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_atoms).Count))) {
          BigInteger _1_i = (BigInteger)_compr_0;
          if (((_1_i).Sign != -1) && ((_1_i) < (new BigInteger((_0_atoms).Count)))) {
            _coll0.Add(Common.__default.Id((_0_atoms).Select(_1_i)));
          }
        }
        return Dafny.Set<Common._IVertex>.FromCollection(_coll0);
      }))())(atoms);
    }
    public static Common._ITranslation Add(Common._ITranslation u, Common._ITranslation v)
    {
      return Common.Translation.create(((u).dtor_x) + ((v).dtor_x), ((u).dtor_y) + ((v).dtor_y), ((u).dtor_z) + ((v).dtor_z));
    }
    public static Common._ITranslation Subtract(Common._ITranslation u, Common._ITranslation v)
    {
      return Common.Translation.create(((u).dtor_x) - ((v).dtor_x), ((u).dtor_y) - ((v).dtor_y), ((u).dtor_z) - ((v).dtor_z));
    }
    public static Common._ITranslation Negate(Common._ITranslation u) {
      return Common.Translation.create((BigInteger.Zero) - ((u).dtor_x), (BigInteger.Zero) - ((u).dtor_y), (BigInteger.Zero) - ((u).dtor_z));
    }
  }

  public interface _ITranslation {
    bool is_Translation { get; }
    BigInteger dtor_x { get; }
    BigInteger dtor_y { get; }
    BigInteger dtor_z { get; }
    _ITranslation DowncastClone();
  }
  public class Translation : _ITranslation {
    public readonly BigInteger _x;
    public readonly BigInteger _y;
    public readonly BigInteger _z;
    public Translation(BigInteger x, BigInteger y, BigInteger z) {
      this._x = x;
      this._y = y;
      this._z = z;
    }
    public _ITranslation DowncastClone() {
      if (this is _ITranslation dt) { return dt; }
      return new Translation(_x, _y, _z);
    }
    public override bool Equals(object other) {
      var oth = other as Common.Translation;
      return oth != null && this._x == oth._x && this._y == oth._y && this._z == oth._z;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      ...
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._z));
      return (int) hash;
    }
    public override string ToString() {
      string s = "Common.Translation.Translation";
      s += "(";
      s += Dafny.Helpers.ToString(this._x);
      s += ", ";
      s += Dafny.Helpers.ToString(this._y);
      s += ", ";
      s += Dafny.Helpers.ToString(this._z);
      s += ")";
      return s;
    }
    private static readonly Common._ITranslation theDefault = create(BigInteger.Zero, BigInteger.Zero, BigInteger.Zero);
    public static Common._ITranslation Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<Common._ITranslation> _TYPE = new Dafny.TypeDescriptor<Common._ITranslation>(Common.Translation.Default());
    public static Dafny.TypeDescriptor<Common._ITranslation> _TypeDescriptor() {
      return _TYPE;
    }
    public static _ITranslation create(BigInteger x, BigInteger y, BigInteger z) {
      return new Translation(x, y, z);
    }
    public static _ITranslation create_Translation(BigInteger x, BigInteger y, BigInteger z) {
      return create(x, y, z);
    }
    public bool is_Translation { get { return true; } }
    public BigInteger dtor_x {
      get {
        return this._x;
      }
    }
    public BigInteger dtor_y {
      get {
        return this._y;
      }
    }
    public BigInteger dtor_z {
      get {
        return this._z;
      }
    }
  ...
  public interface _IAtom {
    bool is_Atom { get; }
    Dafny.ISequence<Dafny.Rune> dtor_atomLabel { get; }
    BigInteger dtor_sym { get; }
    Common._ITranslation dtor_translation { get; }
    _IAtom DowncastClone();
  }
  public class Atom : _IAtom {
    public readonly Dafny.ISequence<Dafny.Rune> _atomLabel;
    public readonly BigInteger _sym;
    public readonly Common._ITranslation _translation;
    public Atom(Dafny.ISequence<Dafny.Rune> atomLabel, BigInteger sym, Common._ITranslation translation) {
      this._atomLabel = atomLabel;
      this._sym = sym;
      this._translation = translation;
    }
    public _IAtom DowncastClone() {
      if (this is _IAtom dt) { return dt; }
      return new Atom(_atomLabel, _sym, _translation);
    }
    public override bool Equals(object other) {
      var oth = other as Common.Atom;
      return oth != null && object.Equals(this._atomLabel, oth._atomLabel) && this._sym == oth._sym && object.Equals(this._translation, oth._translation);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._atomLabel));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._sym));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._translation));
      return (int) hash;
    }
    public override string ToString() {
      string s = "Common.Atom.Atom";
      s += "(";
      s += this._atomLabel.ToVerbatimString(true);
      s += ", ";
      s += Dafny.Helpers.ToString(this._sym);
      s += ", ";
      s += Dafny.Helpers.ToString(this._translation);
      s += ")";
      return s;
    }
    private static readonly Common._IAtom theDefault = create(Dafny.Sequence<Dafny.Rune>.Empty, BigInteger.Zero, Common.Translation.Default());
    public static Common._IAtom Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<Common._IAtom> _TYPE = new Dafny.TypeDescriptor<Common._IAtom>(Common.Atom.Default());
    public static Dafny.TypeDescriptor<Common._IAtom> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IAtom create(Dafny.ISequence<Dafny.Rune> atomLabel, BigInteger sym, Common._ITranslation translation) {
      return new Atom(atomLabel, sym, translation);
    }
    public static _IAtom create_Atom(Dafny.ISequence<Dafny.Rune> atomLabel, BigInteger sym, Common._ITranslation translation) {
      return create(atomLabel, sym, translation);
    }
    public bool is_Atom { get { return true; } }
    public Dafny.ISequence<Dafny.Rune> dtor_atomLabel {
      get {
        return this._atomLabel;
      }
    }
    public BigInteger dtor_sym {
      get {
        return this._sym;
      }
    }
    public Common._ITranslation dtor_translation {
      get {
  ...
  public interface _IVertex {
    bool is_Vertex { get; }
    Dafny.ISequence<Dafny.Rune> dtor_atomLabel { get; }
    BigInteger dtor_sym { get; }
    _IVertex DowncastClone();
  }
  public class Vertex : _IVertex {
    public readonly Dafny.ISequence<Dafny.Rune> _atomLabel;
    public readonly BigInteger _sym;
    public Vertex(Dafny.ISequence<Dafny.Rune> atomLabel, BigInteger sym) {
      this._atomLabel = atomLabel;
      this._sym = sym;
    }
    public _IVertex DowncastClone() {
      if (this is _IVertex dt) { return dt; }
      return new Vertex(_atomLabel, _sym);
    }
    public override bool Equals(object other) {
      var oth = other as Common.Vertex;
      return oth != null && object.Equals(this._atomLabel, oth._atomLabel) && this._sym == oth._sym;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
    ...
    private static readonly Common._IVertex theDefault = create(Dafny.Sequence<Dafny.Rune>.Empty, BigInteger.Zero);
    public static Common._IVertex Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<Common._IVertex> _TYPE = new Dafny.TypeDescriptor<Common._IVertex>(Common.Vertex.Default());
    public static Dafny.TypeDescriptor<Common._IVertex> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IVertex create(Dafny.ISequence<Dafny.Rune> atomLabel, BigInteger sym) {
      return new Vertex(atomLabel, sym);
    }
    public static _IVertex create_Vertex(Dafny.ISequence<Dafny.Rune> atomLabel, BigInteger sym) {
      return create(atomLabel, sym);
    }
    public bool is_Vertex { get { return true; } }
    public Dafny.ISequence<Dafny.Rune> dtor_atomLabel {
      get {
        return this._atomLabel;
      }
  ...
  public interface _IBond {
    bool is_Bond { get; }
    Common._IAtom dtor_a { get; }
    Common._IAtom dtor_b { get; }
    _IBond DowncastClone();
  }
  public class Bond : _IBond {
    public readonly Common._IAtom _a;
    public readonly Common._IAtom _b;
    public Bond(Common._IAtom a, Common._IAtom b) {
      this._a = a;
      this._b = b;
    }
    public _IBond DowncastClone() {
      if (this is _IBond dt) { return dt; }
      return new Bond(_a, _b);
    }
    public override bool Equals(object other) {
      var oth = other as Common.Bond;
    ...
    private static readonly Common._IBond theDefault = create(Common.Atom.Default(), Common.Atom.Default());
    public static Common._IBond Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<Common._IBond> _TYPE = new Dafny.TypeDescriptor<Common._IBond>(Common.Bond.Default());
    public static Dafny.TypeDescriptor<Common._IBond> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IBond create(Common._IAtom a, Common._IAtom b) {
      return new Bond(a, b);
    }
    public static _IBond create_Bond(Common._IAtom a, Common._IAtom b) {
      return create(a, b);
    }
    public bool is_Bond { get { return true; } }
    public Common._IAtom dtor_a {
      get {
        return this._a;
      }
    }
    public Common._IAtom dtor_b {
      get {
        return this._b;
      }
    }
  ...
  public interface _IEdge {
    bool is_Edge { get; }
    Common._IVertex dtor_u { get; }
    Common._IVertex dtor_v { get; }
    Common._ITranslation dtor_shift { get; }
    _IEdge DowncastClone();
  }
  public class Edge : _IEdge {
    public readonly Common._IVertex _u;
    public readonly Common._IVertex _v;
    public readonly Common._ITranslation _shift;
    public Edge(Common._IVertex u, Common._IVertex v, Common._ITranslation shift) {
      this._u = u;
      this._v = v;
      this._shift = shift;
    }
    public _IEdge DowncastClone() {
      if (this is _IEdge dt) { return dt; }
      return new Edge(_u, _v, _shift);
    }
    public override bool Equals(object other) {
      var oth = other as Common.Edge;
      return oth != null && object.Equals(this._u, oth._u) && object.Equals(this._v, oth._v) && object.Equals(this._shift, oth._shift);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      ...
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._shift));
      return (int) hash;
    }
    public override string ToString() {
      string s = "Common.Edge.Edge";
      s += "(";
      s += Dafny.Helpers.ToString(this._u);
      s += ", ";
      s += Dafny.Helpers.ToString(this._v);
      s += ", ";
      s += Dafny.Helpers.ToString(this._shift);
      s += ")";
      return s;
    }
    private static readonly Common._IEdge theDefault = create(Common.Vertex.Default(), Common.Vertex.Default(), Common.Translation.Default());
    public static Common._IEdge Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<Common._IEdge> _TYPE = new Dafny.TypeDescriptor<Common._IEdge>(Common.Edge.Default());
    public static Dafny.TypeDescriptor<Common._IEdge> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IEdge create(Common._IVertex u, Common._IVertex v, Common._ITranslation shift) {
      return new Edge(u, v, shift);
    }
    public static _IEdge create_Edge(Common._IVertex u, Common._IVertex v, Common._ITranslation shift) {
      return create(u, v, shift);
    }
    public bool is_Edge { get { return true; } }
    public Common._IVertex dtor_u {
      get {
        return this._u;
      }
    }
    public Common._IVertex dtor_v {
      get {
        return this._v;
      }
    }
    public Common._ITranslation dtor_shift {
      get {
        return this._shift;
      }
    }
...
namespace QuotientGraph {

  public partial class __default {
    public static void ConstructLQG(Dafny.ISequence<Common._IAtom> atoms, Dafny.ISequence<Common._IBond> bonds, out Dafny.ISet<Common._IVertex> V, out Dafny.ISet<Common._IEdge> E)
    {
      V = Dafny.Set<Common._IVertex>.Empty;
      E = Dafny.Set<Common._IEdge>.Empty;
      Dafny.ISet<Common._IVertex> _0_allNodeIds;
      _0_allNodeIds = Common.__default.AllNodeIds(atoms);
      Dafny.ISet<Common._IVertex> _1_visited;
      _1_visited = _0_allNodeIds;
      Dafny.ISet<Common._IEdge> _2_edges;
      _2_edges = Dafny.Set<Common._IEdge>.FromElements();
      BigInteger _3_atomIdx;
      _3_atomIdx = BigInteger.Zero;
      while ((_3_atomIdx) < (new BigInteger((atoms).Count))) {
        Common._IAtom _4_currentAtom;
        _4_currentAtom = (atoms).Select(_3_atomIdx);
        _3_atomIdx = (_3_atomIdx) + (BigInteger.One);
        Common._IVertex _5_currentNodeId;
        _5_currentNodeId = Common.__default.Id(_4_currentAtom);
        BigInteger _6_bondIdx;
        _6_bondIdx = BigInteger.Zero;
        while ((_6_bondIdx) < (new BigInteger((bonds).Count))) {
          Common._IBond _7_bond;
          _7_bond = (bonds).Select(_6_bondIdx);
          _6_bondIdx = (_6_bondIdx) + (BigInteger.One);
          if (object.Equals((_7_bond).dtor_a, _4_currentAtom)) {
            Common._IVertex _8_neighborNodeId;
            _8_neighborNodeId = Common.__default.Id((_7_bond).dtor_b);
            Common._ITranslation _9_shift;
            _9_shift = Common.__default.Subtract(((_7_bond).dtor_b).dtor_translation, ((_7_bond).dtor_a).dtor_translation);
            Common._IEdge _10_edge;
            _10_edge = Common.Edge.create(_5_currentNodeId, _8_neighborNodeId, _9_shift);
            Common._IEdge _11_invEdge;
            _11_invEdge = Common.Edge.create(_8_neighborNodeId, _5_currentNodeId, Common.__default.Negate(_9_shift));
            if ((!(_2_edges).Contains(_10_edge)) && (!(_2_edges).Contains(_11_invEdge))) {
              _2_edges = Dafny.Set<Common._IEdge>.Union(_2_edges, Dafny.Set<Common._IEdge>.FromElements(_10_edge));
            }
          } else if (object.Equals((_7_bond).dtor_b, _4_currentAtom)) {
            Common._IVertex _12_neighborNodeId;
            _12_neighborNodeId = Common.__default.Id((_7_bond).dtor_a);
            Common._ITranslation _13_shift;
            _13_shift = Common.__default.Subtract(((_7_bond).dtor_a).dtor_translation, ((_7_bond).dtor_b).dtor_translation);
            Common._IEdge _14_edge;
            _14_edge = Common.Edge.create(_5_currentNodeId, _12_neighborNodeId, _13_shift);
            Common._IEdge _15_invEdge;
            _15_invEdge = Common.Edge.create(_12_neighborNodeId, _5_currentNodeId, Common.__default.Negate(_13_shift));
            if ((!(_2_edges).Contains(_14_edge)) && (!(_2_edges).Contains(_15_invEdge))) {
              _2_edges = Dafny.Set<Common._IEdge>.Union(_2_edges, Dafny.Set<Common._IEdge>.FromElements(_14_edge));
            }
          }
        }
      }
    ...
    public static Common._IAtom BaseAtom(Common._IVertex v) {
      return Common.Atom.create((v).dtor_atomLabel, (v).dtor_sym, Common.Translation.create(BigInteger.Zero, BigInteger.Zero, BigInteger.Zero));
    }
    public static Common._IAtom ShiftedAtom(Common._IEdge e) {
      return Common.Atom.create(((e).dtor_v).dtor_atomLabel, ((e).dtor_v).dtor_sym, (e).dtor_shift);
    }
    public static Dafny.ISet<Common._IAtom> BaseAtoms(Dafny.ISet<Common._IVertex> V) {
      return Dafny.Helpers.Id<Func<Dafny.ISet<Common._IVertex>, Dafny.ISet<Common._IAtom>>>((_0_V) => ((System.Func<Dafny.ISet<Common._IAtom>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Common._IAtom>();
        foreach (Common._IVertex _compr_0 in (_0_V).Elements) {
          Common._IVertex _1_v = (Common._IVertex)_compr_0;
          if ((_0_V).Contains(_1_v)) {
            _coll0.Add(QuotientGraph.__default.BaseAtom(_1_v));
          }
        }
        return Dafny.Set<Common._IAtom>.FromCollection(_coll0);
      }))())(V);
    }
    public static Dafny.ISet<Common._IAtom> TranslatedAtoms(Dafny.ISet<Common._IEdge> E) {
      return Dafny.Helpers.Id<Func<Dafny.ISet<Common._IEdge>, Dafny.ISet<Common._IAtom>>>((_0_E) => ((System.Func<Dafny.ISet<Common._IAtom>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Common._IAtom>();
        foreach (Common._IEdge _compr_0 in (_0_E).Elements) {
          Common._IEdge _1_e = (Common._IEdge)_compr_0;
          if (((_0_E).Contains(_1_e)) && (!object.Equals((_1_e).dtor_shift, Common.Translation.create(BigInteger.Zero, BigInteger.Zero, BigInteger.Zero)))) {
            _coll0.Add(QuotientGraph.__default.ShiftedAtom(_1_e));
          }
        }
        return Dafny.Set<Common._IAtom>.FromCollection(_coll0);
      }))())(E);
    }
    public static void ReconstructMolecule(Dafny.ISet<Common._IVertex> V, Dafny.ISet<Common._IEdge> E, out Dafny.ISet<Common._IAtom> atoms, out Dafny.ISet<Common._IBond> bonds)
    {
      atoms = Dafny.Set<Common._IAtom>.Empty;
      bonds = Dafny.Set<Common._IBond>.Empty;
      Dafny.ISet<Common._IAtom> _0_base;
      _0_base = QuotientGraph.__default.BaseAtoms(V);
      Dafny.ISet<Common._IAtom> _1_translated;
      _1_translated = QuotientGraph.__default.TranslatedAtoms(E);
      atoms = Dafny.Set<Common._IAtom>.Union(_0_base, _1_translated);
      bonds = Dafny.Helpers.Id<Func<Dafny.ISet<Common._IEdge>, Dafny.ISet<Common._IBond>>>((_2_E) => ((System.Func<Dafny.ISet<Common._IBond>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Common._IBond>();
        foreach (Common._IEdge _compr_0 in (_2_E).Elements) {
          Common._IEdge _3_e = (Common._IEdge)_compr_0;
          if ((_2_E).Contains(_3_e)) {
            _coll0.Add(Common.Bond.create(QuotientGraph.__default.BaseAtom((_3_e).dtor_u), QuotientGraph.__default.ShiftedAtom(_3_e)));
          }
        }
        return Dafny.Set<Common._IBond>.FromCollection(_coll0);
      }))())(E);
    }
  }
} // end of namespace QuotientGraph
namespace Test__Construct {

  public partial class __default {
    public static BigInteger count__vertices(Dafny.ISet<Common._IVertex> xs)
    {
      BigInteger z = BigInteger.Zero;
      BigInteger _0_count;
      _0_count = BigInteger.Zero;
      Dafny.ISet<Common._IVertex> _1_c;
      _1_c = xs;
      while (!(_1_c).Equals(Dafny.Set<Common._IVertex>.FromElements())) {
        Common._IVertex _2_x;
        foreach (Common._IVertex _assign_such_that_0 in (_1_c).Elements) {
          _2_x = (Common._IVertex)_assign_such_that_0;
          if ((_1_c).Contains(_2_x)) {
            goto after__ASSIGN_SUCH_THAT_0;
          }
        }
        throw new System.Exception("assign-such-that search produced no value");
      after__ASSIGN_SUCH_THAT_0: ;
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Test ")).ToVerbatimString(false));
        Dafny.Helpers.Print((_0_count));
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" vertex ")).ToVerbatimString(false));
        Dafny.Helpers.Print((_2_x));
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
        _1_c = Dafny.Set<Common._IVertex>.Difference(_1_c, Dafny.Set<Common._IVertex>.FromElements(_2_x));
    ...
    public static BigInteger count__edges(Dafny.ISet<Common._IEdge> edges)
    {
      BigInteger z = BigInteger.Zero;
      BigInteger _0_count;
      _0_count = BigInteger.Zero;
      Dafny.ISet<Common._IEdge> _1_c;
      _1_c = edges;
      while (!(_1_c).Equals(Dafny.Set<Common._IEdge>.FromElements())) {
        Common._IEdge _2_x;
        foreach (Common._IEdge _assign_such_that_0 in (_1_c).Elements) {
          _2_x = (Common._IEdge)_assign_such_that_0;
          if ((_1_c).Contains(_2_x)) {
            goto after__ASSIGN_SUCH_THAT_0;
          }
        }
      ...
      after__ASSIGN_SUCH_THAT_0: ;
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Test ")).ToVerbatimString(false));
        Dafny.Helpers.Print((_0_count));
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" edge ")).ToVerbatimString(false));
        Dafny.Helpers.Print((_2_x));
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
        _1_c = Dafny.Set<Common._IEdge>.Difference(_1_c, Dafny.Set<Common._IEdge>.FromElements(_2_x));
        _0_count = (_0_count) + (BigInteger.One);
      }
      z = _0_count;
      return z;
    ...
    {
      Common._IAtom _0_B0;
      _0_B0 = Common.Atom.create(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("B"), BigInteger.One, Common.Translation.create(BigInteger.Zero, BigInteger.Zero, BigInteger.Zero));
      Common._IAtom _1_N00;
      _1_N00 = Common.Atom.create(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("N"), new BigInteger(2), Common.Translation.create(BigInteger.Zero, BigInteger.Zero, BigInteger.Zero));
      Common._IAtom _2_N10;
      _2_N10 = Common.Atom.create(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("N"), new BigInteger(2), Common.Translation.create(BigInteger.One, BigInteger.Zero, BigInteger.Zero));
      Common._IAtom _3_N11;
      _3_N11 = Common.Atom.create(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("N"), new BigInteger(2), Common.Translation.create(BigInteger.One, BigInteger.One, BigInteger.Zero));
      Dafny.ISequence<Common._IAtom> _4_atoms;
      _4_atoms = Dafny.Sequence<Common._IAtom>.FromElements(_0_B0, _1_N00, _2_N10, _3_N11);
      Dafny.ISequence<Common._IBond> _5_bonds;
      _5_bonds = Dafny.Sequence<Common._IBond>.FromElements(Common.Bond.create(_0_B0, _1_N00), Common.Bond.create(_0_B0, _2_N10), Common.Bond.create(_0_B0, _3_N11));
      Dafny.ISet<Common._IVertex> _6_V;
      Dafny.ISet<Common._IEdge> _7_E;
      Dafny.ISet<Common._IVertex> _out0;
      Dafny.ISet<Common._IEdge> _out1;
      QuotientGraph.__default.ConstructLQG(_4_atoms, _5_bonds, out _out0, out _out1);
      _6_V = _out0;
      _7_E = _out1;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Results:\n")).ToVerbatimString(false));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("|V| = ")).ToVerbatimString(false));
      Dafny.Helpers.Print((new BigInteger((_6_V).Count)));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" unique vertices\n")).ToVerbatimString(false));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("|E| = ")).ToVerbatimString(false));
      Dafny.Helpers.Print((new BigInteger((_7_E).Count)));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" edges\n")).ToVerbatimString(false));
      BigInteger _8_vertex__count;
      BigInteger _out2;
      _out2 = Test__Construct.__default.count__vertices(_6_V);
      _8_vertex__count = _out2;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Vertex count: ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_8_vertex__count));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      BigInteger _9_edges__count;
      BigInteger _out3;
      _out3 = Test__Construct.__default.count__edges(_7_E);
      _9_edges__count = _out3;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Edges count: ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_9_edges__count));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
    }
  }
} // end of namespace Test__Construct
namespace _module {

} // end of namespace _module
class __CallToMain {
  public static void Main(string[] args) {
    Dafny.Helpers.WithHaltHandling(() => Test__Construct.__default._Main(Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.UnicodeFromMainArguments(args)));
  }
}
